...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Draw project cell contents directly
    this.items.forEach((item, i) => {
      const x = (i % this.atlasSize) * cellSize;
      const y = Math.floor(i / this.atlasSize) * cellSize;

      // Draw elegant dark blue gradient background
      ...
      const radialGrad = ctx.createRadialGradient(
        x + cellSize / 2, y + cellSize / 2, 20,
        x + cellSize / 2, y + cellSize / 2, cellSize / 2 - 20
      );
      radialGrad.addColorStop(0, 'rgba(59, 130, 246, 0.18)');
      radialGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
      ctx.fillStyle = radialGrad;
      ctx.fillRect(x, y, cellSize, cellSize);

      // Glowing circular stroke
      ctx.strokeStyle = 'rgba(59, 130, 246, 0.75)';
      ctx.lineWidth = 16;
      ctx.beginPath();
      ctx.arc(x + cellSize / 2, y + cellSize / 2, cellSize / 2 - 24, 0, Math.PI * 2);
      ctx.stroke();

      // Letter drawing
      const letter = item.title ? item.title.charAt(0).toUpperCase() : '?';
      ctx.font = 'bold 230px "JetBrains Mono", monospace';
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(canvas, items, handleActiveItem, setIsMoving, sk => sk.run(), scale);
    sketchRef.current = sketch;

    const handleResize = () => { if (sketchRef.current) sketchRef.current.resize(); };
    window.addEventListener('resize', handleResize);
    handleResize();

    return () => {
      window.removeEventListener('resize', handleResize);
      if (sketchRef.current) sketchRef.current.stop();
    };
  }, [items, scale]);
